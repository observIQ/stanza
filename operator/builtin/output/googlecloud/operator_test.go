...
func TestFlusher(t *testing.T) {
	...
	testEntry := &entry.Entry{Record: "test record"}
...
func TestBufferFailure(t *testing.T) {
	...
	buffer.On("ReadChunk", mock.Anything).Run(bufferFunc).Return(nil, nil, errors.New("first failure")).Once()
	buffer.On("ReadChunk", mock.Anything).Return(nil, nil, errors.New("continued failures"))
...
// MockBuffer is a mock type for the Buffer interface
...
func (_m *MockBuffer) ReadChunk(_a0 context.Context) ([]*entry.Entry, buffer.Clearer, error) {
	ret := _m.Called(_a0)
